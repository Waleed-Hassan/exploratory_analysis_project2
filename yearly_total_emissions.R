...
summary_data <- (tapply(measurements$Emissions, measurements$year, sum))/10e3
...
plot(x=names(summary_data), 
     ...
     pch=16,
     col=3,
     ylim=c(min(summary_data)-80, max(summary_data)+80))

#label data poins
for(i in 1:length(summary_data)){
text(names(summary_data)[i], summary_data[i] + 50, round(summary_data[i]))
}
